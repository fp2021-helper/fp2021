...
let identifier is_valid_first_char = 
  ...
  | _                -> false
  in
  (empty *> satisfy is_valid_first_char) >>= (fun fst -> 
  let take_func = 
    match fst with
    | '_' -> many1
    | _ -> many
  in
  (take_func (satisfy is_char_id)) >>= (fun inner ->
...
(* const parsing *)
let cunsint = 
  trim (take_while1 is_digit) >>= (fun helper ->
  return ( Int.of_string helper ) >>| cint)

let cint = 
  (trim (option "" (token "+" <|> token "-"))) >>= (fun sign ->
  (take_while1 is_digit) >>= (fun helper ->
...
let pack = 
  let key d = 
    ...
    in
    let efun = trim @@ lift2 efun (kwd "fun" *> many pattern <* arrow) (d.expr d) in
    let ematch = 
      let fst_case = lift2 ccase (bar *> pattern <* arrow) (d.expr d) in
      let other_cases = lift2 ccase (bar *> pattern <* arrow) (d.expr d) in
  ...
  let op d = 
    fix 
    @@ fun _self -> 
    let lst = trim @@ between lsb rsb @@ sep_by semi (d.expr d) in
    let prim = 
      trim
      @@ choice [ lst >>| elist; uns_const >>| econst; id >>| evar; parens @@ d.expr d ]
    in
    let helper op pl pr = 
      let rec go acc = 
      ...
      pl >>= go
    in
    let app_op = trim ((chainl1 prim eapp) <|> (lift2 (fun id prim -> EConstr (id, prim)) constr_id prim)) in
    ...
    trim @@ helper disj conj_op @@ d.key d 
  in
  { key; tuple; expr; op }
...
let tyexp = 
  ...
  let list = 
    prim >>= (fun lst_ty ->
    (many1 (empty1 *> token "list")) >>= (fun lst ->
    let rec wrap acc n =
      match n with
      | 0 -> acc
      | _ -> wrap (TList acc) (n - 1)
    in
    return @@ wrap lst_ty (List.length lst)))
  in
  let tup =
...
let prog =
  many1 (trim @@ decl <* trim (many (trim semisemi)))
