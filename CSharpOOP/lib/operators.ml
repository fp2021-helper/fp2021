...
let ( === ) left right =
  ...
  | VObjectReference left_value, VObjectReference right_value -> (
      ...
      | ( ObjectReference
            { class_key = _; ext_interface = _; field_references_table = _; number = left_number },
          ObjectReference
            { class_key = _; ext_interface = _; field_references_table = _; number = right_number }
        ) ->
...
let%test _ =
  VObjectReference
    (ObjectReference
       { class_key = "Test"; ext_interface = None; field_references_table = KeyMap.empty; number = 1 })
  === VObjectReference
        (ObjectReference
           {
             class_key = "Test";
