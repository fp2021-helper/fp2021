...
module ClassLoader (M : MONADERROR) = struct
  ...
  let rec build_new_children_fields (seq_fields : (string * table_field) Seq.t)
    ...
    | Seq.Cons ((key, parent_field), next) ->
        let new_fields_table =
          ...
          | None ->
              if is_private parent_field.field_modifiers then children.fields_table
              else KeyMap.add parent_field.key parent_field children.fields_table
  ...
  let many_check_method_override ht parent children =
    let not_change_access_modifiers parent_m children_m =
      if
        (is_public parent_m.method_modifiers && is_public children_m.method_modifiers)
        || (is_protected parent_m.method_modifiers && is_protected children_m.method_modifiers)
        || (is_private parent_m.method_modifiers && is_private children_m.method_modifiers)
      then true
      else false
    in
    let check_children_method_override :
     ...
     fun parent (children_method_key, children_method) ->
      ...
      | true -> (
          ...
          | Some parent_method -> (
              match
                is_virtual parent_method.method_modifiers || is_abstract parent_method.method_modifiers
